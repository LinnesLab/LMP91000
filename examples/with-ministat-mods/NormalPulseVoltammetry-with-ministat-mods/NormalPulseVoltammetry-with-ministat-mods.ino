...
void setup()
{
  ...
  //runNPV(gain,period,width,startV,endV,quietTime,current)
            ...
            int16_t startV, int16_t endV, uint32_t quietTime, uint8_t range)
{
  ...
  Serial.println("Voltage(mV),ADC(3.3V 10-bit," + current + "adcVal with var,newGetCurrentMethod");
  ...
  for (uint8_t i = 0; i < pulses; i++)
  {
    ...
    double adcVal = analogRead(A0);
    adcVal = -1*(adcVal-512)+512;
    
    Serial.print((uint16_t)(opVolt*TIA_BIAS[abs(nextV)]*(nextV/abs(nextV))));
    Serial.print(",");
    Serial.print(analogRead(A0));
    Serial.print(",");
    Serial.print(pow(10,range)*pStat.getCurrent(pStat.getOutput(A0), opVolt/1000.0, resolution));
    Serial.print(",");
    Serial.print(adcVal);
    Serial.print(",");
    Serial.println(pow(10,range)*pStat.getCurrent(adcVal, opVolt/1000.0, resolution));
